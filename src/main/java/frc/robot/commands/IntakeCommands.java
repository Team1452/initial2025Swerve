...
public class IntakeCommands {
   ...
   * @return a command that runs the intake rotation until the desired state is reached, 
   *         then stops the intake.
   */
  private static Command rotateIntakeCommand(Intake intake, boolean open) {
    ...
    BooleanSupplier condition = open 
        ? intake::getIntakeState
        : () -> !intake.getIntakeState();
    return Commands.sequence(
        Commands.run(() -> intake.rotateIntake(open), intake)
            .until(condition),
   ...
   */
  private static Command spinSuckerCommand(Intake intake, boolean suck) {
    return Commands.sequence(
        Commands.run(() -> intake.spinSucker(suck), intake),
  ...
  public static Command runIntakeRoutine(Intake intake) {
    return Commands.sequence(
        ...
        .handleInterrupt(
            // If the command is interrupted, spit out the coral and close the intake ("panic mode").
