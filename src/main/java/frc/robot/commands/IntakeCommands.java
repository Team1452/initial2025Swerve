...
public class IntakeCommands {
  ...
  public static Command openIntake(Intake intake) {
    // This command rotates the intake until it reaches the desired open angle defined in
    // IntakeConstants.
    // The command runs the intake motor (via rotateIntake()) until the error is within a tolerance
    // (here, 0.05 units),
  ...
  public static Command closeIntake(Intake intake) {
    ...
    // Might be marginally better to use .andThen instead of sequence. But we'll see.
    return Commands.sequence(
        Commands.run(
                () -> {
  ...
  public static Command runIntakeRoutine(Intake intake) {
    return Commands.sequence(
        ...
        .handleInterrupt( // If the command is interrupted, spit out the coral and close the intake,
            // "panic mode".
